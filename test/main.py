...
def preprocess_song(song_input, mdx_model_params, song_id,mdxnet_models_dir='mdxnet_models',output_dir='song_outputs', input_type='yt'):
    ...
    if not os.path.exists(os.path.join(f"{output_dir}/{song_id}",f"{orig_song_path.replace('.mp3','')}_Vocals.wav")) and not os.path.exists(os.path.join(f"{output_dir}/{song_id}",f"{orig_song_path.replace('.mp3','')}_Instrumental.wav")):
        ...
        vocals_path, instrumentals_path = run_mdx(mdx_model_params, song_output_dir, os.path.join(mdxnet_models_dir, 'UVR-MDX-NET-Inst_HQ_2.onnx'), orig_song_path, denoise=True, keep_orig=keep_orig)
    else:
